...
Явные и неявные преобразования типов не поддерживаются.
...
- Строка преобразуется в тип переменной:
  - Для `int`: ожидается целое число (возможно со знаком). Если преобразование невозможно – ошибка
...
- Если тип возврата не `void`, инструкция `return` должна содержать выражение, тип которого точно совпадает с объявленным типом
